 ...
 */
public class SearchFragment extends BottomSheetDialogFragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener, IParser<WSResponse>, ISingleSelectionClick, IMultipleSelectionClick {
...
//    CustomEditText edtTitle;
    ...
    public static SearchFragment newInstance(Bundle filtersBundle) {
        SearchFragment fragment = new SearchFragment();
        fragment.setArguments(filtersBundle);
        return fragment;
    }
...
//                rootView = inflater.inflate(R.layout.fragment_search, null);
    ...
    private void initialize() {
        ...
        if (!TextUtils.isEmpty(activity.cityId)) {
            ...
            requestNeighbourhoodList(true, !TextUtils.isEmpty(location) && !location.equalsIgnoreCase("-1") ? location : "");
        } else {
    ...
    private void getBundleData() {
        if (getArguments() != null) {
            ...
            title = savedBundle.getString("title");
            max_price = savedBundle.getString("max_price");
            min_price = savedBundle.getString("min_price");
            sortType = savedBundle.getString("sort_type");
            leasing_type = savedBundle.getString("leasing_type");
            location = savedBundle.getString("location");
            check_in = savedBundle.getString("check_in");
            neighbourhood = savedBundle.getString("neighbourhood");
            amenities = savedBundle.getString("amenities");
            section = savedBundle.getString("section");
            suitable_for = savedBundle.getString("suitable_for");
            size = savedBundle.getString("size");
            to = savedBundle.getString("to");
            from = savedBundle.getString("from");
            availibility = savedBundle.getString("availibility");
            lat = savedBundle.getString("lat");
            lng = savedBundle.getString("lng");
            property_type_id = savedBundle.getString("property_type_id");
        }
    }

    @Override
    @OnClick({R.id.tv_location, R.id.tv_neighbourhood, R.id.tv_leasing, R.id.tv_size, R.id.tv_amenities, R.id.tv_to_time, R.id.tv_from_time,
            R.id.tv_checkin_date, R.id.tv_sections, R.id.tv_suitable_for, R.id.txtDone, R.id.txtCancel})
    public void onClick(View v) {
        switch (v.getId()) {
...
//                dismiss();
    ...
    private void onLocationEvent() {
        if (cityLocationList != null && cityLocationList.size() > 0) {
            PopupUtil.showSingleSelection(activity, getString(R.string.select_location), cityLocationList, null, null, null, this);
        } else {
    ...
    private void onSearchEvent() {
        ...
        activity.replaceFragment(ListViewFragment.newInstance(bundle), false, false, false, true);
//        clearData();
        dismiss();
...
//        edtTitle.setText("");
        ...
        savedBundle = null;
        ListViewFragment.filtersBundle = null;
    }
    ...
    private void makeSelectedIdTrue() {
        if (cityLocationList != null && cityLocationList.size() > 0 && location != null) {
            for (int i = 0; i < cityLocationList.size(); i++) {
                if (cityLocationList.get(i).getLocationId().equalsIgnoreCase(location)) {
                    cityLocationList.get(i).setSelected(true);
                    tvLocation.setText(cityLocationList.get(i).getLocationName());
                } else cityLocationList.get(i).setSelected(false);
//                cityLocationList.get(i).setSelected(cityLocationList.get(i).getLocationId().equalsIgnoreCase(location));
        ...
        if (!TextUtils.isEmpty(check_in))
            tvCheckinDate.setText(check_in);
        if (!TextUtils.isEmpty(max_price))
            edtMaxPrice.setText(max_price);
        if (!TextUtils.isEmpty(min_price))
            edtMinPrice.setText(min_price);
        if (!TextUtils.isEmpty(sortType))
            if (sortType.equalsIgnoreCase("price asc")) {
                rbLowestPrice.setChecked(true);
            } else if (sortType.equalsIgnoreCase("distance asc")) {
        ...
        if (!TextUtils.isEmpty(leasing_type))
            tvLeasing.setText(leasing_type);
        ...
        if (!TextUtils.isEmpty(section))
            tvSections.setText(section);
        if (!TextUtils.isEmpty(suitable_for))
            tvSuitableFor.setText(suitable_for);
